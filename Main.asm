...
; Description: Copy the mushroom sprite data to the sprite memory location for  both mushroom sprites.
; Inputs: None
; Outputs: 
;    -Places the mushroom 1 sprite in memory
;    -Places the mushroom 2 sprite in memory
...
COPY_MUSHROOM_LOOP
        ...
        ; Alters Sprite Data and pointer
        lda #$BB
        sta $07F9
        lda #7
        sta $D028
...
; Description: When a collision is detected between the cat sprite and another object. It changes the cat sprite's color to red.
; Inputs: None
; Outputs: 
...
COLLISION_DETECTED
        lda $D01E
        and #$03
        bne COLLISION_WITH_MUSHROOM

        lda #$02
        sta $D027
        rts

; Description: When a collision is detected between the cat sprite and another sprite, disables the collided sprite and checks if it is the last mushroom.
; Inputs: None
; Outputs: 
;    - Updates sprite enable register ($D015)
;    - Calls COLLISION_WITH_LAST_MUSHROOM if last sprite
COLLISION_WITH_MUSHROOM
        lda $D015
        lsr
        sta $D015
        ...
        lda $D015
        cmp #$01
        beq COLLISION_WITH_LAST_MUSHROOM

        rts

; Description: Handles the last mushroom collision. Updates its enable bit and moves it to a new Y position using NOT(current_y) <<< 1.
; Inputs:
;    - Uses the Cat Sprites Y value on screen ($D001)
; Outputs:
;    - Updates sprite enable register ($D015)
;    - Sprite Y position updated ($D003)
COLLISION_WITH_LAST_MUSHROOM
        sec
        lda $D015
        rol
        sta $D015

        jsr UPDATE_MUSHROOM_COLOR
...
; Description: Updates the mushroom color. Ensures it never becomes 6 (background color).
; Inputs:
;    - Uses sprite color ($D028)
; Outputs:
;    - Updates sprite color ($D028)
UPDATE_MUSHROOM_COLOR
        lda $D028
        cmp #5
        beq SKIP_COLOR6
...
; Description: Updates the mushroom color to value 7.
; Inputs: none
; Outputs:
;    - Updates sprite color ($D028)
SKIP_COLOR6
